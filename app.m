...
tg = uitabgroup(f, 'Units','pixels');
tg.Position = tg_get_pos(fw, fh);
...
Load_btn = uicontrol(tab_main, 'Style','PushButton', ...
        'Units','pixels',...
        'String','Загрузить');
...
RG_ECG_get_pos = @(fw, fh) [ ...
        DX,         DY*3 + (fh - 50 - DY*4) / 3 * 2, ...
        fw - DX*2,  (fh - DY*4) / 3 ...
    ];
RG_ECG_axes = axes(tab_main, 'Units', 'pixels',...
...
RG_ABP_get_pos = @(fw, fh) [ ...
        DX,         DY*2 + (fh - 50 - DY*4) / 3 * 1, ...
        fw - DX*2,  (fh - DY*4) / 3 ...
    ];
RG_ABP_axes = axes(tab_main, 'Units', 'pixels',...
...
POWER_get_pos = @(fw, fh) [ ...
        DX,         DY*1 + (fh - 50 - DY*4) / 3 * 0, ...
        fw - DX*2,  (fh - DY*4) / 3 ...
    ];
POWER_axes = axes(tab_main, 'Units', 'pixels',...
...
RRrg_axes = axes( ...
        tab_ellipse, ...
        'Units', 'pixels',...
        'Position', RRrg_axes_get_pos(fw, fh));
...
SSrg_axes = axes( ...
        tab_ellipse, ...
        'Units', 'pixels',...
        'Position', SSrg_axes_get_pos(fw, fh));
...
RRpsd_axes_get_pos = @(fw, fh) [ ...
        DX + SETTINGS_WIDTH + DX + (fw - DX*5)/4 + DX + (fw - DX - SETTINGS_WIDTH - DX - (fw - DX*5)/4 - DX - DX*2) / 2 + DX,	...
        fh - DY - (fh - DY*4)/3, ...
        (fw - DX - SETTINGS_WIDTH - DX - (fw - DX*5)/4 - DX - DX*2) / 2, ...
        (fh - DY*4) / 3 ...
    ];
RRpsd_axes = axes( ...
        tab_psd, ...
...
drag_point_RR_a = DragPoint(RRscatter, f);
drag_point_RR_b = DragPoint(RRscatter, f);
drag_point_RR_center = DragPoint(RRscatter, f);

f.WindowButtonMotionFcn = @on_f2_mouse_moution;
f.WindowButtonDownFcn = @on_f2_mouse_down;
f.WindowButtonUpFcn = @on_f2_mouse_up;
...
drag_point_SS_a = DragPoint(SSscatter, f);
drag_point_SS_b = DragPoint(SSscatter, f);
drag_point_SS_center = DragPoint(SSscatter, f);
...
function on_main_figure_size_changed(s, ~)
    ...
    RRrg_axes.Position = RRrg_axes_get_pos(fw, fh);
    ...
    SSrg_axes.Position = SSrg_axes_get_pos(fw, fh);
...
function count_for_selected_span(to_recount)
    ...
    if contains(to_recount, 'RR')
        axes(RRrg_axes); cla; hold on; grid on;
    ...
    if contains(to_recount, 'SS')
        axes(SSrg_axes); cla; hold on; grid on;
